...
<div class="max-w-4xl mx-auto animate-slide-up">
    <div class="bg-white/80 dark:bg-slate-800/60 glass rounded-3xl p-8 shadow-sm border border-white/20 dark:border-white/5">
        <div class="flex items-center gap-4 mb-8 border-b border-gray-200 dark:border-white/5 pb-6">
            ...
            <div>
                <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Role: {{ $role->name }}</h2>
                <p class="text-slate-500 dark:text-slate-400">Modify role details and permissions.</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="group">
                 ...
                 <div class="group">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Slug</label>
                    <div class="relative">
                        <input type="text" name="slug" value="{{ old('slug', $role->slug) }}" class="w-full pl-10 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600 text-slate-800 dark:text-slate-200">
            ...
            <div class="group">
                ...
                <textarea name="description" rows="2" class="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 outline-none transition-all text-slate-800 dark:text-slate-200">{{ old('description', $role->description) }}</textarea>
            </div>
                    ...
                    @foreach($permissions as $module => $modulePermissions)
                     <div class="animate-fade-in-up" style="animation-delay: {{ $loop->index * 50 }}ms">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-white/10 cursor-pointer hover:bg-gray-50 dark:hover:bg-white/5 transition-all group hover:border-indigo-200 dark:hover:border-indigo-500/30 hover:shadow-sm {{ in_array($permission->id, $rolePermissions) ? 'bg-indigo-50/50 dark:bg-indigo-500/5 border-indigo-100 dark:border-indigo-500/20' : '' }}">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }} class="permission-checkbox w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-gray-300 dark:border-slate-600 bg-gray-100 dark:bg-slate-700 group-hover:border-indigo-400 transition-colors">
