...
<div class="space-y-6 animate-slide-up">
    ...
    <div class="flex flex-col md:flex-row justify-between gap-4 bg-white/80 dark:bg-slate-800/60 glass rounded-3xl p-6 shadow-sm border border-white/20 dark:border-white/5">
        <form action="{{ route('admin.permissions.index') }}" method="GET" class="flex gap-4 flex-1">
             <div class="relative w-full max-w-md">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search permissions..." class="w-full pl-10 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600 text-slate-800 dark:text-slate-200">
                <i data-lucide="search" class="w-5 h-5 absolute left-3 top-3.5 text-slate-400"></i>
            </div>
            <div class="relative">
                 <select name="module" onchange="this.form.submit()" class="appearance-none w-full pl-4 pr-10 py-3 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-white/10 focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 outline-none transition-all text-slate-600 dark:text-slate-300 cursor-pointer">
                    <option value="">All Modules</option>
                    @foreach($permissions->unique('module') as $p)
                        <option value="{{ $p->module }}" {{ request('module') == $p->module ? 'selected' : '' }}>{{ $p->module_label }}</option>
                    @endforeach
                </select>
